...
const PurchaseAccountManagement = () => {
  ...
  useEffect(() => {
        ...
        if (isEditModeRef.current && editingAccount === undefined) {
          ...
          if (form) {
            ...
            if (!hasValue) {
              e.preventDefault()
              e.stopPropagation()
              e.stopImmediatePropagation() // PurchaseAccountForm의 핸들러가 처리하지 못하도록
          ...
          // 필드에 값이 있으면 이벤트를 전달하여 PurchaseAccountForm의 F2 핸들러가 저장 처리
        } else {
    ...
    // capture phase에서 먼저 처리하여 PurchaseAccountForm의 핸들러보다 먼저 실행
